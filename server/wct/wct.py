...
# import tensorflow as tf # updated here
...
class WCT(object):
    ...
    def __init__(self, checkpoints, relu_targets, vgg_path, device='/gpu:0',
        ...
        with graph.device(device):
            ...
            # for relu_target, checkpoint_dir in zip(relu_targets, checkpoints): update here
            ...
            for relu_target, checkpoint_dir in zip(relu_targets, checkpoints):
                ...
                if ckpt and ckpt.model_checkpoint_path:
                    ...
                    try:
                        saver = tf.train.Saver(var_list=relu_vars)
                        saver.restore(self.sess, ckpt.model_checkpoint_path)
                        print('Successfully loaded {}'.format(relu_target))
                    except Exception as e:
                        print('Failed to load {}: {}'.format(relu_target, str(e)))
                        print('Continuing with random weights for {}...'.format(relu_target))
                else:
                    print('No checkpoint found for {}, using random weights'.format(relu_target))
