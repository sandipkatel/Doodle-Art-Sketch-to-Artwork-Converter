    ...
    for idx,module in enumerate(t7.modules):
        ...
        if module._typename == b'nn.SpatialReflectionPadding':
            ...
            x = Conv2D(filters, kernel_size, padding='valid', activation=None, name=name,
                        # kernel_initializer=lambda shape, dtype: K.constant(weight, shape=shape, dtype='float32'), # updated here
                        kernel_initializer=lambda shape, dtype: tf.constant(weight, shape=shape, dtype='float32'),
                        # bias_initializer=lambda shape, dtype: K.constant(bias, shape=shape, dtype='float32'), # updated here
                        bias_initializer=lambda shape, dtype: tf.constant(bias, shape=shape, dtype='float32'), 
                        trainable=False)(x)
